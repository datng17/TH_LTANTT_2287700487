...
def verify_certificate_chain(cert_to_verify, chain):
    try:
        # Note: A real implementation is more complex. This is a simplified check.
        # It assumes the chain is ordered: [intermediate_cert, root_cert]
        current_cert = cert_to_verify
        for issuer_cert in chain:
            ...
            # Check if the issuer name of current cert matches the subject name of the issuer cert
            if current_cert.issuer != issuer_cert.subject:
                raise Exception("Certificate chain issuer/subject mismatch.")
